<script lang="ts">
  ...
  import { GET_ALL_SIPS_V2, GET_ALL_USERS } from '$lib/graphql/queries';
  ...
  onMount(async () => {
    ...
    hasAccess = true;
    await Promise.all([loadSips(), loadUsers()]);
  });
  ...
  function getRootFieldValue(sip: any, fieldName: string): string {
    if (!sip.rootElement?.fields) return '';
    const field = sip.rootElement.fields.find((f: any) => f.name === fieldName);
    return field?.value || '';
  }
...
<svelte:head>
  <title>SIPs - Admin - Archiving System</title>
</svelte:head>
...
<style>
  ...
  .access-denied h1 { margin: 0 0 1rem 0; color: #1e293b; }
