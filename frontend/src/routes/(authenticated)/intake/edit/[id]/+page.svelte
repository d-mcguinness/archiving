<script lang="ts">
  ...
  import { GET_INTAKE, GET_ALL_USERS, GET_ALL_INTAKES_V2 } from '$lib/graphql/queries';
  ...
  const UPDATE_INTAKE_STATUS = gql`
    mutation UpdateIntakeStatusV2($intakeId: ID!, $status: IntakeStatus!) {
      updateIntakeStatusV2(intakeId: $intakeId, status: $status) {
        id
        status
        updatedAt
      }
  ...
  const GENERATE_INTAKE = gql`
    mutation GenerateIntake($intakeId: ID!) {
      generateIntake(intakeId: $intakeId)
    }
  `;

  const DELETE_INTAKE = gql`
  ...
  $: intakeId = $page.params.id || $page.params.intakeId;
  ...
  onMount(async () => {
    await Promise.all([loadIntake(), loadUsers(), loadDocuments()]);
  ...
  async function loadIntake() {
    try {
      ...
      const result = await client.query({
        query: GET_INTAKE,
        variables: { id: intakeId },
        fetchPolicy: 'network-only'
      });
      sip = result?.data?.getIntake;
      if (!sip) {
      ...
      error = e instanceof Error ? e.message : 'Failed to load Intake';
    } finally {
      loading = false;
    }
  }
  ...
  async function handleStatusChange(newStatus: string) {
    ...
    try {
      const result = await client.mutate({
        mutation: UPDATE_INTAKE_STATUS,
        variables: { intakeId: sip.id, status: newStatus }
      });
      sip = { ...sip, status: result.data.updateIntakeStatusV2.status, updatedAt: result.data.updateIntakeStatusV2.updatedAt };
  ...
  async function handleGenerate() {
    ...
    try {
      await client.mutate({
        mutation: GENERATE_INTAKE,
        variables: { intakeId: sip.id }
      });
      toasts.success('Intake generated successfully');
      await loadIntake();
    } catch (e) {
      toasts.error(`Failed to generate Intake: ${e instanceof Error ? e.message : 'Unknown error'}`);
    } finally {
      generating = false;
    }
  }
  ...
  async function handleDelete() {
    if (!confirm(`Are you sure you want to delete Intake "${sip.title}"?`)) return;
    deleting = true;
    try {
      await client.mutate({
        mutation: DELETE_INTAKE,
        variables: { id: sip.id },
        refetchQueries: [{ query: GET_ALL_INTAKES_V2 }]
      });
      toasts.success('Intake deleted');
      goto(getIntakesPath());
    } catch (e) {
      toasts.error(`Failed to delete Intake: ${e instanceof Error ? e.message : 'Unknown error'}`);
    } finally {
      deleting = false;
    }
  }
  ...
  async function loadDocuments() {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/documents?intakeId=${intakeId}`, { headers: { ...authHeaders() } });
  ...
  async function loadUnassociatedDocuments() {
    try {
      ...
      const result = await response.json();
      if (result.success) {
        unassociatedDocuments = (result.documents || []).filter((d: any) => !d.intakeId && !d.archiveId);
      }
    } catch (e) {
  ...
  async function handleUploadDocument() {
    ...
    try {
      ...
      if (uploadResult.success && uploadResult.document?.id) {
        await fetch(
          `${API_BASE}/api/documents/${uploadResult.document.id}/associate-sip?intakeId=${intakeId}`,
          { method: 'POST', headers: { ...authHeaders() } }
        );
        toasts.success('Document uploaded and associated with Intake');
        closeDocumentModal();
        await loadDocuments();
      } else {
  ...
  async function handleLinkDocument() {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/documents/${selectedDocumentId}/associate-sip?intakeId=${intakeId}`,
        { method: 'POST', headers: { ...authHeaders() } }
      );
      const result = await response.json();
      if (result.success) {
        toasts.success('Document linked to Intake');
        closeDocumentModal();
        await loadDocuments();
      } else {
  ...
  async function handleRemoveDocument(docId: string) {
    if (!confirm('Remove this document from the Intake? (The document will not be deleted)')) return;
    try {
      const response = await fetch(
        `${API_BASE}/api/documents/${docId}/disassociate-sip`,
        { method: 'POST', headers: { ...authHeaders() } }
      );
      const result = await response.json();
      if (result.success) {
        toasts.success('Document removed from Intake');
        await loadDocuments();
      } else {
  ...
  async function handleDownloadDocument(docId: string, fileName: string) {
    try {
      const response = await fetch(`${API_BASE}/api/documents/${docId}/file`, { headers: { ...authHeaders() } });
...
<svelte:head>
  <title>{sip ? `Edit Intake - ${sip.title}` : 'Edit Intake'}</title>
</svelte:head>
...
<div class="sip-edit-page">
  ...
  {:else if error}
    <div class="error-state">
      <p>{error}</p>
      <button class="btn btn-secondary" on:click={() => goto(getIntakesPath())}>Back to Intakes</button>
    </div>
  {:else if sip}
    <!-- Header -->
    <div class="page-header">
      <div class="header-left">
        <button class="btn-back" on:click={() => goto(getIntakesPath())}>← Back</button>
        <div>
          <span class="eyebrow">Intake</span>
          <h1>{sip.title}</h1>
          <div class="header-meta">
            <span class="badge pink">{sip.standard}</span>
      ...
      <div class="header-actions">
        <div class="btn-group">
          <button class="btn btn-generate" on:click={handleGenerate} disabled={generating}>
            {generating ? 'Generating...' : 'Generate Intake'}
          </button>
          <a href="/intake/create?standard={encodeURIComponent(graphqlToKey[sip.standard] || sip.standard)}" class="btn btn-generate btn-group-right" title="Create a new {graphqlToKey[sip.standard] || sip.standard} Intake">
            +
          </a>
        </div>
    ...
    <div class="content-grid">
      <!-- Left: Intake Details -->
      <div class="panel">
        <h2 class="panel-title">Intake Details</h2>
    ...
    <div class="panel">
      ...
      {:else}
        <p class="no-data" style="margin-top:1rem;">No documents associated with this Intake.</p>
      {/if}
    </div>
...
<style>
  ...
  /* Buttons — .btn-primary, .btn-secondary and .btn-danger come from the
     ...
     The transition is kept because .btn also rides on an <a>, which the
     global <button> element rule does not reach. */
  .btn {
  ...
  .btn-group .btn:first-child { border-radius: 0.65rem 0 0 0.65rem; }
  ...
  /* Badges — the .badge base and hue variants are global; only this page's
     tighter pill and the Intake-specific statuses stay local. */
  .badge { padding: 0.2rem 0.6rem; }

  .badge.draft { background: var(--arc-chip-amber-bg); color: var(--arc-chip-amber-ink); }
  .badge.submitted { background: var(--arc-chip-indigo-bg); color: var(--arc-chip-indigo-ink); }
  .badge.validated { background: var(--arc-chip-green-bg); color: var(--arc-chip-green-ink); }
  .badge.accepted { background: var(--arc-chip-green-bg); color: var(--arc-chip-green-ink); }
