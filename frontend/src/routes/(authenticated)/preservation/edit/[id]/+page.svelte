...
<style>
  ...
  /* Badges — the .badge base and hue variants are global; only this page's
     tighter pill and the Preservation-specific statuses stay local. */
  .badge { padding: 0.2rem 0.6rem; }

  .badge.draft { background: var(--arc-chip-amber-bg); color: var(--arc-chip-amber-ink); }
  .badge.building { background: var(--arc-chip-indigo-bg); color: var(--arc-chip-indigo-ink); }
  .badge.validated { background: var(--arc-chip-green-bg); color: var(--arc-chip-green-ink); }
  .badge.stored { background: var(--arc-chip-violet-bg); color: var(--arc-chip-violet-ink); }
