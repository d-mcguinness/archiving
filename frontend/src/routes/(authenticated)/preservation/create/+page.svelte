<script lang="ts">
  ...
  import { GET_ALL_USERS, CREATE_PRESERVATION, GET_ALL_PRESERVATIONS, GET_ALL_ARCHIVES, GET_ARCHIVES_BY_TENANT } from '$lib/graphql/queries';
  ...
  onMount(async () => {
    ...
    const qIntakeId = params.get('sourceIntakeId');
    ...
    if (qIntakeId) sourceIntakeId = qIntakeId;
    ...
    // Pre-select standard from query param (e.g. from /preserve page)
  ...
  async function onStandardChange() {
    ...
    try {
      ...
      // Find the Preservation root entity
  ...
  function genId() {
    return `Preservation-${sourceArchiveId || sourceIntakeId || Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
  }
  ...
  function applyAllDefaults() {
    ...
    if (!aipTitle) {
      ...
      aipTitle = `${standardLabel} Preservation - ${new Date().toLocaleDateString()}`;
    }
    if (!aipDescription) {
      aipDescription = `Preservation package created using the ${selectedStandard?.label || 'selected'} standard.`;
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const elementIdentifier = fieldValues['packageID'] || fieldValues['systemID'] || fieldValues['aipID'] || fieldValues['objectIdentifierValue'] || fieldValues['bagName'] || fieldValues['metsID'] || fieldValues['eadID'] || fieldValues['descriptionID'] || fieldValues['modsID'] || fieldValues['resourceIdentifier'] || `Preservation-${Date.now()}`;
      ...
      const input: Record<string, any> = {
        ...
        elementIdentifier,
        entityName: preservationEntityDef.name,
        entityType: preservationEntityDef.type,
      ...
      if (sourceIntakeId) {
        input.sourceIntakeId = sourceIntakeId;
      }
      ...
      } else if (newPreservationId) {
        goto(`/preservation/edit/${newPreservationId}`);
      } else {
  ...
  <div class="aip-header">
    <div class="aip-header-top">
      <div>
        ...
        <p class="subtitle">Build an Preservation by selecting an archive</p>
      </div>
      {#if selectedStandard}
        <button
          type="button"
    ...
    <form on:submit|preventDefault={handleSubmit}>
      ...
      <section class="form-section">
        ...
        <div class="form-group">
          <label for="archiveSelect">Archive for Preservation <span class="req">*</span></label>
      ...
      {#if selectedArchiveId}
        <section class="form-section">
          ...
          <div class="form-group">
            <label for="sourceIntakeId">Source Intake ID (optional)</label>
            <input type="text" id="sourceIntakeId" bind:value={sourceIntakeId} placeholder="Link to originating Intake (optional)" />
          </div>
        </section>
        ...
        {:else if preservationEntityDef}
          <section class="form-section">
            <h2 class="section-title">
              3. {selectedStandard?.preservationLabel} Fields
            </h2>

            {#if preservationEntityDef.description}
              <p class="entity-description">{preservationEntityDef.description}</p>
            {/if}

            {#if preservationEntityDef.note}
              <div class="entity-note">
                {preservationEntityDef.note}
              </div>
            {/if}
            ...
            {#if preservationEntityDef.children && preservationEntityDef.children.length > 0}
              <div class="children-info">
                ...
                <p class="helper-text">After creating this Preservation, you can add the following child elements:</p>
      ...
      <div class="form-actions">
        ...
        <button
          ...
          disabled={submitting || !selectedArchiveId || !preservationEntityDef}
        >
