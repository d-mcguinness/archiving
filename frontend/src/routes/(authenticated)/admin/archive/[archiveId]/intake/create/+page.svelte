<script lang="ts">
  ...
  import { GET_ALL_USERS, CREATE_INTAKE, GET_ALL_INTAKES_V2, PREFILL_INTAKE_FIELDS } from '$lib/graphql/queries';
    ...
    <form on:submit|preventDefault={handleSubmit}>
      ...
      {:else if intakeEntityDef}
        <section class="form-section">
          ...
          {#if intakeEntityDef.description}<p class="entity-description">{intakeEntityDef.description}</p>{/if}
          {#if intakeEntityDef.note}<div class="entity-note">{intakeEntityDef.note}</div>{/if}
...
<style>
  .sip-container { max-width: 960px; margin: 2rem auto; background: var(--arc-card, #fff); padding: 2.5rem; border-radius: 1rem; box-shadow: var(--arc-shadow-card, 0 1px 2px rgba(15, 23, 42, 0.04)); border: 1px solid var(--arc-line, #e8edf3); }
  .sip-header { margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 1px solid var(--arc-line, #e8edf3); }
  .sip-header-top { display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem; }
  .sip-header h1 { margin: 0 0 0.5rem 0; color: var(--arc-ink, #0f172a); font-size: 1.75rem; font-weight: 700; }
  ...
  .fields-group-toggle { display: flex; align-items: center; justify-content: space-between; width: 100%; background: none; border: none; cursor: pointer; padding: 0; margin-bottom: 0; color: inherit; box-shadow: none; }
  ...
  .child-tag { padding: 0.25rem 0.75rem; background: var(--arc-chip-indigo-bg); color: var(--arc-chip-indigo-ink); border-radius: 9999px; font-size: 0.75rem; font-weight: 600; box-shadow: none; }
