<script lang="ts">
  ...
  import { GET_ALL_USERS, CREATE_SIP, GET_ALL_SIPS_V2, PREFILL_SIP_FIELDS } from '$lib/graphql/queries';
    ...
    <form on:submit|preventDefault={handleSubmit}>
      ...
      {:else if sipEntityDef}
        <section class="form-section">
          ...
          {#if sipEntityDef.description}<p class="entity-description">{sipEntityDef.description}</p>{/if}
          {#if sipEntityDef.note}<div class="entity-note">{sipEntityDef.note}</div>{/if}
