<script lang="ts">
  ...
  import { GET_ALL_USERS, CREATE_AIP, GET_ALL_AIPS, GET_SIPS_BY_TENANT_V2, PREFILL_SIP_FIELDS } from '$lib/graphql/queries';
  ...
  $: filteredSips = archive ? sips.filter((s: any) => s.standard === archive.standard) : sips;
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (archive?.tenantId) {
        const sipsResult = await client.query({ query: GET_SIPS_BY_TENANT_V2, variables: { tenantId: archive.tenantId.toString() }, fetchPolicy: 'network-only' });
        sips = sipsResult?.data?.getSipsByTenantV2 || [];
      }
  ...
  async function loadSchema() {
    ...
    } catch (e) { error = e instanceof Error ? e.message : 'Failed to load schema'; aipEntityDef = null; }
    finally { loadingSchema = false; }
  }

  function genId() { return `AIP-${data.archiveId}-${Math.random().toString(36).substring(2, 8)}`; }
  ...
  function getStandardDefaults(standardKey: string, title: string, description: string): Record<string, string> {
    ...
    const map: Record<string, Record<string, string>> = {
      ...
      'OAIS': { aipID: id, title: title || 'New Archival Information Package', description: description || '', creationDate: today, version: '1.0', accessRights: 'restricted', packageType: 'AIP' },
      ...
      'E-ARK': { packageID: id, title: title || 'New Archival Information Package', description: description || '', profile: 'https://earkcsip.dilcis.eu/profile/E-ARK-CSIP.xml', contentInformationType: 'MIXED', oaisPackageType: 'AIP', creationDate: today, creator: userName, preservationLevel: 'full', representationCount: '1' },
    };
  ...
  function applyAllDefaults() {
    ...
    if (!aipTitle) aipTitle = `${selectedStandard?.label || 'Archive'} AIP - ${new Date().toLocaleDateString()}`;
    if (!aipDescription) aipDescription = `Archival Information Package created using the ${selectedStandard?.label || 'selected'} standard.`;
    ...
    for (const field of aipEntityDef.fields) { const val = defaults[field.name]; if (val !== undefined) fieldValues[field.name] = val; }
  ...
  async function handleFilesProcessed(e: CustomEvent<{ files: File[], metadata: any }>) {
    ...
    try {
      ...
      for (const { name, value } of (result?.data?.prefillSipFields || [])) { if (aipEntityDef?.fields?.some((f: any) => f.name === name)) fieldValues[name] = value; }
      fieldValues = fieldValues;
      if (files.length === 1 && metadata.filename) { const baseName = metadata.filename.replace(/\.[^.]+$/, ''); if (!aipTitle || aipTitle.startsWith('AIP -')) aipTitle = `AIP - ${baseName}`; }
  ...
  async function handleSubmit() {
    ...
    try {
      const aipContent = JSON.stringify({ aipType: selectedStandard!.aipLabel, standard: selectedStandard!.key, entity: aipEntityDef.name, fields: fieldValues }, null, 2);
      const elementIdentifier = fieldValues['systemID'] || fieldValues['aipID'] || fieldValues['packageID'] || fieldValues['objectIdentifierValue'] || fieldValues['bagName'] || fieldValues['metsID'] || fieldValues['eadID'] || fieldValues['descriptionID'] || fieldValues['modsID'] || fieldValues['resourceIdentifier'] || `AIP-${Date.now()}`;
      const fields = aipEntityDef.fields.map((fieldDef: any) => ({ name: fieldDef.name, label: fieldDef.label, type: fieldDef.type, value: fieldValues[fieldDef.name] || '' }));
      const input: any = { userId: selectedUserId, title: aipTitle, description: aipDescription || null, content: aipContent, standard: selectedStandard!.graphql, elementIdentifier, entityName: aipEntityDef.name, entityType: aipEntityDef.type, elementTitle: aipTitle, elementDescription: aipDescription || null, createdBy: selectedUserId, fields };
      if (selectedSipId) input.sourceSipId = selectedSipId;
      await client.mutate({ mutation: CREATE_AIP, variables: { input }, refetchQueries: [{ query: GET_ALL_AIPS }], awaitRefetchQueries: true });
      toasts.add(`AIP "${aipTitle}" created successfully using ${selectedStandard!.label}`, 'success');
      goto('/admin/archives');
    } catch (e) { error = e instanceof Error ? e.message : 'Failed to create AIP'; toasts.add(`Failed to create AIP: ${error}`, 'error'); }
...
<svelte:head><title>Create AIP - {archive?.title || 'Archive'} - Arcana</title></svelte:head>

<div class="aip-container">
  <Breadcrumb items={[{ label: 'Admin', href: '/admin' }, { label: 'Archives', href: '/admin/archives' }, { label: archive?.title || 'Archive' }, { label: 'Create AIP' }]} />

  <div class="aip-header"><div class="aip-header-top"><div><h1>Create Archival Information Package</h1>{#if archive}<p class="subtitle">Archive: {archive.title} ({archive.standard})</p>{/if}</div>
    ...
    <form on:submit|preventDefault={handleSubmit}>
      ...
      <section class="form-section">
        ...
        <div class="form-row">
          ...
          <div class="form-group"><label for="aipTitle">AIP Title <span class="req">*</span></label><input type="text" id="aipTitle" bind:value={aipTitle} required placeholder="Enter AIP title" /></div>
        </div>
        ...
        <div class="form-group"><label for="sourceSip">Source SIP (optional)</label><select id="sourceSip" bind:value={selectedSipId}><option value="">-- No source SIP --</option>{#each filteredSips as sip}<option value={sip.id}>#{sip.id} - {sip.title} ({sip.status})</option>{/each}</select><span class="field-hint">Optionally link this AIP to a source SIP.</span></div>
      </section>

      {#if selectedStandard}
        <section class="form-section"><h2 class="section-title">Upload Files</h2><p class="section-hint">Drop files to auto-populate AIP metadata fields.</p><FileDropZone on:filesProcessed={handleFilesProcessed} on:filesCleared={handleFilesCleared} disabled={prefilling} />{#if prefilling}<p class="prefill-status">Auto-populating fields...</p>{/if}{#if prefilledFromFiles}<p class="prefill-success">Fields populated from file metadata.</p>{/if}</section>
      {/if}
      ...
      {:else if aipEntityDef}
        <section class="form-section">
          <h2 class="section-title">{selectedStandard?.aipLabel} Fields</h2>
          {#if aipEntityDef.description}<p class="entity-description">{aipEntityDef.description}</p>{/if}
          {#if aipEntityDef.note}<div class="entity-note">{aipEntityDef.note}</div>{/if}
          ...
          {#if aipEntityDef.children && aipEntityDef.children.length > 0}<div class="children-info"><h3 class="fields-group-title">Available Child Entities</h3><p class="helper-text">Click to create AIP and add a child element:</p><div class="children-tags">{#each aipEntityDef.children as child}<button type="button" class="child-tag child-tag-clickable" on:click={() => handleCreateAndAddChild(child)} disabled={submitting}>{child}</button>{/each}</div></div>{/if}
        </section>
      {/if}

      <div class="form-actions"><button type="button" class="btn btn-secondary" on:click={handleCancel}>Cancel</button><button type="submit" class="btn btn-primary" disabled={submitting || !aipEntityDef}>{submitting ? 'Creating AIP...' : 'Create AIP'}</button></div>
    </form>
  {/if}
</div>
