<script lang="ts">
  ...
  export let data: { id?: string; tenantId?: string } = {};

  function getArchivesPath() {
    const { role, tenantId } = get(auth);
    if (role === 'TENANT' && tenantId) return `/tenants/${tenantId}/archives`;
    if (role === 'ADMIN') return '/archives';
    return '/';
  }
  ...
  // Get archive ID from data (load function) or route parameter
  $: archiveId = data?.id || $page.params.id || $page.params.archiveId;
  ...
  function fillDefaults() {
    if (!editingElement) return;
    const scheme = schemes.find((s: any) => s.name === editingElement.entityName);
    ...
    for (const field of scheme.fields) {
      ...
      if (type === 'date') {
        ...
        defaults[field.name] = 'en';
      } else if (name.includes('format')) {
        defaults[field.name] = 'application/pdf';
      } else if (name.includes('algorithm') || name.includes('checksum')) {
        defaults[field.name] = 'SHA-256';
      } else if (name.includes('level')) {
        defaults[field.name] = 'Full';
      } else if (name.includes('rights') || name.includes('access')) {
        defaults[field.name] = 'Open';
      } else if (name.includes('creator') || name.includes('opprettetav') || name.includes('author')) {
        defaults[field.name] = 'System';
      } else if (name.includes('description') || name.includes('beskrivelse')) {
        defaults[field.name] = `Default ${field.label || field.name}`;
      } else if (name.includes('title') || name.includes('tittel') || name.includes('name')) {
        defaults[field.name] = editingElement.title || `Default ${field.label || field.name}`;
      } else {
        defaults[field.name] = `Sample ${field.label || field.name}`;
      }
    }

    // Only fill empty fields
    for (const [key, val] of Object.entries(defaults)) {
      if (!elementForm.fieldValues[key]) {
        elementForm.fieldValues[key] = val;
      }
    }
    elementForm.fieldValues = elementForm.fieldValues;

    if (!editingElement.title) {
      editingElement.title = `${editingElement.entityName} - Default`;
    }
    if (!editingElement.description) {
      editingElement.description = `Auto-filled default for ${editingElement.entityName}`;
    }

    toasts.info('Default values filled');
  }
...
{#if showElementModal && editingElement}
  ...
  >
    ...
    >
      ...
      <div class="modal-actions">
        {#if devmode}
          <button class="btn btn-defaults" on:click={fillDefaults}>
            Fill Defaults
          </button>
        {/if}
...
<style>
  ...
  .back-btn:hover {
    background: none;
    color: var(--arc-violet-deep, #7c3aed);
    transform: none;
    box-shadow: none;
  }
