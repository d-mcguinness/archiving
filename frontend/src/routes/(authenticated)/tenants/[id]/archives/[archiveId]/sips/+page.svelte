<script lang="ts">
  ...
  import { GET_SIPS_BY_TENANT_V2, GET_ALL_USERS, GET_TENANT } from '$lib/graphql/queries';
  ...
  onMount(async () => {
    await Promise.all([loadArchive(), loadTenant(), loadSips(), loadUsers()]);
    loading = false;
  });
  ...
  async function loadSips() {
    try { const r = await client.query({ query: GET_SIPS_BY_TENANT_V2, variables: { tenantId: data.tenantId }, fetchPolicy: 'network-only' }); sips = r?.data?.getSipsByTenantV2 || []; } catch (e) { toasts.error('Failed to load SIPs'); }
  }
  ...
  // Filter SIPs to only those matching this archive's standard
  $: filteredSips = archive?.standard
    ? sips.filter((s: any) => s.standard === archive.standard)
    : sips;
</script>

<svelte:head><title>SIPs - {archive?.title || 'Archive'} - Arcana</title></svelte:head>

<div class="page">
  {#if loading}
    <div class="loading"><div class="spinner"></div><p>Loading...</p></div>
  {:else}
    <Breadcrumb context={{ tenantId: data.tenantId, tenantName: tenant?.displayName || tenant?.name }} items={[ { label: 'Archives', href: `/tenants/${data.tenantId}/archives` }, { label: archive?.title || 'Archive', href: `/tenants/${data.tenantId}/archives/${data.archiveId}` }, { label: 'SIPs' } ]} />
    ...
    {#if filteredSips.length === 0}
      <div class="empty"><h3>No SIPs found for this archive</h3><p>Create a SIP with the {archive?.standard || ''} standard to get started.</p></div>
    {:else}
      <div class="table-container">
        <table class="data-table">
          <thead><tr><th>ID</th><th>Title</th><th>Standard</th><th>Status</th><th>Owner</th><th>Created</th><th>Actions</th></tr></thead>
          <tbody>
            {#each filteredSips as sip (sip.id)}
              <tr>
                ...
                <td class="actions-cell"><a href="/tenants/{data.tenantId}/archives/{data.archiveId}/sips/{sip.id}/edit" class="btn-action btn-edit">✏️ Edit</a></td>
              </tr>
            {/each}
          </tbody>
        </table>
