<script lang="ts">
  ...
  onMount(async () => {
    await Promise.all([loadArchive(), loadTenant(), loadAips(), loadUsers()]);
    loading = false;
  });
  ...
  async function loadAips() {
    try {
      ...
      toasts.error('Failed to load AIPs');
    }
  }
  ...
  $: filteredAips = archive?.standard
    ? aips.filter((a: any) => a.standard === archive.standard)
    : aips;
</script>

<svelte:head>
  <title>AIPs - {archive?.title || 'Archive'} - Arcana</title>
</svelte:head>
...
<div class="page">
  ...
  {:else}
    ...
    {#if filteredAips.length === 0}
      <div class="empty">
        <h3>No AIPs found for this archive</h3>
        <p>Create an AIP with the {archive?.standard || ''} standard to get started.</p>
      </div>
    {:else}
      <div class="table-container">
        <table class="data-table">
          ...
          <tbody>
            {#each filteredAips as aip (aip.id)}
              <tr>
                ...
                <td><span class="badge standard">{aip.standard}</span></td>
                ...
                <td class="actions-cell">
                  <a href="/aip/edit/{aip.id}" class="btn-action btn-edit">✏️ Edit</a>
                </td>
              </tr>
            {/each}
          </tbody>
...
<style>
  ...
  .spinner { border: 4px solid #f3f4f6; border-top: 4px solid #6366f1; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; }
  ...
  .btn-create { padding: 0.65rem 1.25rem; background: #6366f1; color: white; border-radius: 0.5rem; text-decoration: none; font-weight: 600; font-size: 0.875rem; transition: background 0.2s; white-space: nowrap; }
  ...
  .empty h3 { margin: 0 0 0.5rem; color: #1e293b; }
  ...
  .data-table thead { background: #eef2ff; border-bottom: 2px solid #c7d2fe; }
  .data-table th { padding: 1rem; text-align: left; font-weight: 600; color: #3730a3; font-size: 0.875rem; text-transform: uppercase; letter-spacing: 0.05em; }
  ...
  .aip-desc { font-size: 0.825rem; color: #64748b; margin-top: 0.15rem; }
