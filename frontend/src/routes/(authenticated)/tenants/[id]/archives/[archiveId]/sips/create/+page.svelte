<script lang="ts">
  ...
  import { GET_ALL_USERS, CREATE_SIP, GET_ALL_SIPS_V2, GET_TENANT, PREFILL_SIP_FIELDS } from '$lib/graphql/queries';
  ...
  function genId() {
    return `SIP-${data.archiveId}-${Math.random().toString(36).substring(2, 8)}`;
  }
  ...
  async function handleFilesProcessed(e: CustomEvent<{ files: File[], metadata: any }>) {
    ...
    try {
      ...
      if (files.length === 1 && metadata.filename) {
        ...
        if (!sipTitle || sipTitle.startsWith('SIP -')) {
          sipTitle = `SIP - ${baseName}`;
        }
      }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (pendingChildEntity && newSipId) {
        ...
        goto(`/tenants/${data.tenantId}/archives/${data.archiveId}/sips/${newSipId}/edit`);
      } else {
        goto(`/tenants/${data.tenantId}/archives/${data.archiveId}/sips`);
      }
    } catch (e) {
...
<svelte:head><title>Create SIP - {archive?.title || 'Archive'} - Arcana</title></svelte:head>
...
<div class="sip-container">
  <Breadcrumb context={{ tenantId: data.tenantId, tenantName: tenant?.displayName || tenant?.name }} items={[
    ...
    { label: 'SIPs', href: `/tenants/${data.tenantId}/archives/${data.archiveId}/sips` },
    { label: 'Create' }
  ]} />
  ...
  {:else}
    <form on:submit|preventDefault={handleSubmit}>
      ...
      {:else if sipEntityDef}
        <section class="form-section">
          <h2 class="section-title">{selectedStandard?.sipLabel} Fields</h2>
      ...
      <div class="form-actions">
        ...
        <button type="submit" class="btn btn-primary" disabled={submitting || !sipEntityDef}>
