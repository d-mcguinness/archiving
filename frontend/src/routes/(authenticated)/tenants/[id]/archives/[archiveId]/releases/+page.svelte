<script lang="ts">
  ...
  import { GET_RELEASES_BY_TENANT, GET_ALL_USERS, GET_TENANT } from '$lib/graphql/queries';
  ...
  onMount(async () => {
    await Promise.all([loadArchive(), loadTenant(), loadReleases(), loadUsers()]);
    loading = false;
  });
  ...
  async function loadReleases() {
    try { const r = await client.query({ query: GET_RELEASES_BY_TENANT, variables: { tenantId: data.tenantId }, fetchPolicy: 'network-only' }); dips = r?.data?.getReleasesByTenant || []; } catch (e) { toasts.error('Failed to load Releases'); }
  }
  ...
  $: filteredReleases = archive?.standard
    ? dips.filter((d: any) => d.standard === archive.standard)
    : dips;
</script>

<svelte:head><title>Releases - {archive?.title || 'Archive'} - Arcana</title></svelte:head>

<div class="page">
  {#if loading}
    <div class="loading"><div class="spinner"></div><p>Loading...</p></div>
  {:else}
    <Breadcrumb context={{ tenantId: data.tenantId, tenantName: tenant?.displayName || tenant?.name }} items={[ { label: 'Archives', href: `/tenants/${data.tenantId}/archives` }, { label: archive?.title || 'Archive', href: `/tenants/${data.tenantId}/archives/${data.archiveId}` }, { label: 'Releases' } ]} />
    ...
    {#if filteredReleases.length === 0}
      <div class="empty"><h3>No Releases found for this archive</h3><p>Create a Release with the {archive?.standard || ''} standard to get started.</p></div>
    {:else}
      <div class="table-container table-card">
        <table class="data-table arc-table">
          ...
          <tbody>
            {#each filteredReleases as dip (dip.id)}
              <tr>
                <td class="id-cell">{dip.id}</td>
                <td class="title-cell"><div class="item-title">{dip.title}</div>{#if dip.description}<div class="item-desc">{dip.description}</div>{/if}</td>
                <td><span class="badge standard orange">{dip.standard}</span></td>
                <td><span class="badge status-{getStatusClass(dip.status)}">{dip.status}</span></td>
                <td class="owner-cell">{getUserName(dip.ownerId)}</td>
                <td class="date-cell">{new Date(dip.createdAt).toLocaleDateString()}</td>
                <td class="actions-cell"><a href="/release/edit/{dip.id}" class="btn-action btn-edit btn-chip indigo">✏️ Edit</a></td>
              </tr>
            {/each}
          </tbody>
        </table>
...
<style>
  ...
  /* .table-card + .arc-table from the kit; this page adds the orange accent
     rail, horizontal scrolling and a roomier cell scale. */
  .table-container { overflow-x: auto; border-top: 3px solid #f97316; }
  ...
  .title-cell { min-width: 200px; font-size: 1rem; } .item-title { font-weight: 600; color: var(--arc-ink); } .item-desc { font-size: 0.825rem; color: var(--arc-muted); margin-top: 0.15rem; }
  .owner-cell, .date-cell { color: var(--arc-muted); font-size: 0.875rem; white-space: nowrap; }
  .actions-cell { white-space: nowrap; }
  /* Pill geometry + the orange standard tint come from the global .badge kit;
     these Release status names are page-specific. */
  .status-draft { background: var(--arc-chip-amber-bg); color: var(--arc-chip-amber-ink); } .status-prepared { background: var(--arc-chip-indigo-bg); color: var(--arc-chip-indigo-ink); } .status-disseminated { background: var(--arc-chip-green-bg); color: var(--arc-chip-green-ink); } .status-expired { background: var(--arc-chip-slate-bg); color: var(--arc-chip-slate-ink); } .status-rejected { background: var(--arc-chip-red-bg); color: var(--arc-chip-red-ink); }
