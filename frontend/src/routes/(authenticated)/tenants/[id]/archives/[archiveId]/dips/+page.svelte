<script lang="ts">
  ...
  onMount(async () => {
    await Promise.all([loadArchive(), loadTenant(), loadDips(), loadUsers()]);
    loading = false;
  });
  ...
  async function loadDips() {
    try { const r = await client.query({ query: GET_DIPS_BY_TENANT, variables: { tenantId: data.tenantId }, fetchPolicy: 'network-only' }); dips = r?.data?.getDipsByTenant || []; } catch (e) { toasts.error('Failed to load DIPs'); }
  }
  ...
  $: filteredDips = archive?.standard
    ? dips.filter((d: any) => d.standard === archive.standard)
    : dips;
</script>

<svelte:head><title>DIPs - {archive?.title || 'Archive'} - Arcana</title></svelte:head>

<div class="page">
  {#if loading}
    <div class="loading"><div class="spinner"></div><p>Loading...</p></div>
  {:else}
    <Breadcrumb context={{ tenantId: data.tenantId, tenantName: tenant?.displayName || tenant?.name }} items={[ { label: 'Archives', href: `/tenants/${data.tenantId}/archives` }, { label: archive?.title || 'Archive', href: `/tenants/${data.tenantId}/archives/${data.archiveId}` }, { label: 'DIPs' } ]} />
    ...
    {#if filteredDips.length === 0}
      <div class="empty"><h3>No DIPs found for this archive</h3><p>Create a DIP with the {archive?.standard || ''} standard to get started.</p></div>
    {:else}
      <div class="table-container">
        <table class="data-table">
          <thead><tr><th>ID</th><th>Title</th><th>Standard</th><th>Status</th><th>Owner</th><th>Created</th><th>Actions</th></tr></thead>
          <tbody>
            {#each filteredDips as dip (dip.id)}
              <tr>
                ...
                <td class="actions-cell"><a href="/dip/edit/{dip.id}" class="btn-action btn-edit">✏️ Edit</a></td>
              </tr>
            {/each}
          </tbody>
        </table>
