...
<div class="page">
  ...
  {:else}
    ...
    <div class="page-header">
      ...
      <a href="/tenants/{data.tenantId}/archives/{data.archiveId}/intakes/create" class="btn-create btn-primary">+ Create Intake</a>
    </div>
          ...
          <tbody>
            {#each filteredIntakes as sip (sip.id)}
              <tr>
                ...
                <td><span class="badge standard pink">{sip.standard}</span></td>
                ...
                <td class="actions-cell"><a href="/tenants/{data.tenantId}/archives/{data.archiveId}/intakes/{sip.id}/edit" class="btn-action btn-edit btn-chip indigo">✏️ Edit</a></td>
              </tr>
            {/each}
          </tbody>
        </table>
...
<style>
  ...
  /* .table-card + .arc-table from the kit; this page adds the pink accent
     rail, horizontal scrolling and a roomier cell scale. */
  .table-container { overflow-x: auto; border-top: 3px solid #ec4899; }
  ...
  /* Pill geometry + the pink standard tint come from the global .badge kit;
     these Intake status names are page-specific. */
