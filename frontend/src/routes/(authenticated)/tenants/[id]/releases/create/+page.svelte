<script lang="ts">
  ...
  import { GET_ALL_USERS, CREATE_RELEASE, GET_ALL_RELEASES, GET_ARCHIVES_BY_TENANT, GET_TENANT, GET_PRESERVATIONS_BY_TENANT, PREFILL_INTAKE_FIELDS } from '$lib/graphql/queries';
  ...
  $: filteredPreservations = selectedArchive
    ? aips.filter((a: any) => a.standard === selectedArchive.standard)
    : aips;
  ...
  onMount(async () => {
    ...
    if (currentRole === 'ADMIN') {
      ...
      hasAccess = false;
      loading = false;
      toasts.error('You do not have permission to create Releases');
      goto(`/tenants/${data.tenantId}/releases`);
      return;
    }

    loading = true;
    try {
      const [tenantResult, archivesResult, usersResult, aipsResult] = await Promise.all([
        ...
        client.query({ query: GET_PRESERVATIONS_BY_TENANT, variables: { tenantId: data.tenantId }, fetchPolicy: 'network-only' })
      ]);
  ...
  async function onArchiveSelect() {
    if (!selectedArchiveId) {
      selectedArchive = null; selectedStandard = null; releaseEntityDef = null; schemaData = null;
      fieldValues = {}; dipTitle = ''; dipDescription = ''; selectedUserId = ''; selectedPreservationId = '';
    ...
    dipTitle = selectedArchive.title ? `Release - ${selectedArchive.title}` : '';
    dipDescription = selectedArchive.description || '';
  ...
  async function loadSchema() {
    ...
    try {
      ...
      error = e instanceof Error ? e.message : 'Failed to load schema'; releaseEntityDef = null;
    } finally { loadingSchema = false; }
  }

  function genId() { return `Release-${selectedArchiveId || Date.now()}-${Math.random().toString(36).substring(2, 8)}`; }
  ...
  async function handleFilesProcessed(e: CustomEvent<{ files: File[], metadata: any }>) {
    ...
    try {
      ...
      for (const { name, value } of (result?.data?.prefillIntakeFields || [])) {
        if (releaseEntityDef?.fields?.some((f: any) => f.name === name)) fieldValues[name] = value;
      }
      fieldValues = fieldValues;
      if (files.length === 1 && metadata.filename) {
        const baseName = metadata.filename.replace(/\.[^.]+$/, '');
        if (!dipTitle || dipTitle.startsWith('Release -')) dipTitle = `Release - ${baseName}`;
      }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await client.mutate({ mutation: CREATE_RELEASE, variables: { input }, refetchQueries: [{ query: GET_ALL_RELEASES }], awaitRefetchQueries: true });
  ...
  function handleCancel() { goto(`/tenants/${data.tenantId}/releases`); }
</script>

<svelte:head><title>Create Release - {tenant?.displayName || tenant?.name || 'Tenant'} - Arcana</title></svelte:head>

{#if !hasAccess && !loading}
  <div class="access-denied"><div class="access-denied-icon">🚫</div><h1>Access Denied</h1><p>You don't have permission to create Releases.</p><p class="redirect-message">Redirecting...</p></div>
{:else}
  <div class="dip-container">
    <Breadcrumb context={{ tenantId: data.tenantId, tenantName: tenant?.displayName || tenant?.name }} items={[{ label: 'Releases', href: `/tenants/${data.tenantId}/releases` }, { label: 'Create' }]} />

    <div class="dip-header"><div class="dip-header-top"><div><span class="eyebrow">Release</span><h1>Create Release package</h1>{#if tenant}<p class="subtitle">Tenant: {tenant.displayName || tenant.name}</p>{/if}</div>
    ...
    {:else}
      <form on:submit|preventDefault={handleSubmit}>
        <section class="form-section">
          ...
          <div class="form-group">
            ...
            <select id="archiveSelect" bind:value={selectedArchiveId} on:change={onArchiveSelect} required>
              ...
              {#each archives as archive}<option value={archive.id}>#{archive.id} - {archive.title} ({archive.standard})</option>{/each}
            </select>
            <span class="field-hint">The standard and defaults will be derived from the selected archive.</span>
          </div>
          {#if selectedStandard && schemaData}<div class="standard-info"><strong>{schemaData.fullName}</strong><span class="standard-ref">{schemaData.reference}</span><p>{schemaData.description}</p></div>{/if}
        </section>
        ...
        {#if selectedArchiveId}
          ...
          {#if selectedStandard}
            <section class="form-section">
              ...
              <p class="section-hint">Drop files to auto-populate Release metadata fields from file properties.</p>
          ...
          {:else if releaseEntityDef}
            <section class="form-section">
              <h2 class="section-title">3. {selectedStandard?.releaseLabel} Fields</h2>
        ...
        <div class="form-actions"><button type="button" class="btn btn-secondary" on:click={handleCancel}>Cancel</button><button type="submit" class="btn btn-primary" disabled={submitting || !selectedArchiveId || !releaseEntityDef}>{submitting ? 'Creating Release...' : 'Create Release'}</button></div>
      </form>
    {/if}
  </div>
{/if}
...
<style>
  ...
  .dip-container { max-width: 960px; margin: 2rem auto; background: var(--arc-card); padding: 2.5rem; border-radius: 1rem; box-shadow: var(--arc-shadow-card, 0 1px 2px rgba(15, 23, 42, 0.04)); border: 1px solid var(--arc-line); }
  .dip-header { margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 1px solid var(--arc-line); }
  .dip-header-top { display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem; }
  .dip-header h1 { margin: 0 0 0.5rem 0; color: var(--arc-ink); font-size: 1.75rem; font-weight: 700; }
  ...
  /* Spinner chrome is the global .spinner; only the size, accent and slower
     sweep are local. */
  .loading-state .spinner { width: 2.5rem; height: 2.5rem; border-top-color: var(--arc-link); animation: spin 1s linear infinite; }
  ...
  .fields-group-toggle { display: flex; align-items: center; justify-content: space-between; width: 100%; background: none; border: none; box-shadow: none; cursor: pointer; padding: 0; color: inherit; }
