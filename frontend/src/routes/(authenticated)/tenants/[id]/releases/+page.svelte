<script lang="ts">
  ...
  let selectedReleaseForExtract: any = null;
  let extractPassword = '';
  ...
  onMount(async () => {
    ...
    await Promise.all([loadTenant(), loadReleases(), loadUsers()]);
  });
  ...
  function closeExtractDialog() {
    ...
    selectedReleaseForExtract = null;
    extractPassword = '';
    extractError = null;
    extracting = false;
  }
  ...
  async function handleExtract() {
    if (!extractPassword || !selectedReleaseForExtract) return;
    extracting = true;
    extractError = null;
    try {
      const response = await fetch(`${API_BASE}/api/archives/${selectedReleaseForExtract.id}/extract`, {
        method: 'POST',
      ...
      const filename = filenameMatch?.[1] || `dip_${selectedReleaseForExtract.id}_export.json`;
      ...
      closeExtractDialog();
      toasts.success('Release extracted successfully');
    } catch (e) {
      extractError = e instanceof Error ? e.message : 'Failed to extract Release';
    } finally {
      extracting = false;
    }
  }
...
<svelte:head>
  <title>{tenant ? `${tenant.displayName || tenant.name} - ` : ''}Releases - Archiving System</title>
</svelte:head>
...
<div class="tenant-dips-page">
  ...
  {:else}
    ...
    <div class="page-header">
      ...
      <a href="/tenants/{data.tenantId}/releases/create" class="btn-create btn-primary">+ Create Release</a>
    </div>
    ...
    {:else if dips.length === 0}
      <div class="empty-state">
        ...
        <p>This tenant doesn't have any Release packages yet.</p>
        <a href="/tenants/{data.tenantId}/releases/create" class="btn-primary-link btn-primary">Create Release</a>
      </div>
    {:else}
            ...
            {#each dips as dip (dip.id)}
              <tr>
                ...
                <td class="standard-cell">
                  <span class="badge orange standard-badge">{dip.standard}</span>
                </td>
                ...
                <td class="actions-cell">
                  <a href="/release/edit/{dip.id}" class="btn-action btn-edit btn-chip indigo">
                    ✏️ Edit
                  </a>
                  <button class="btn-action btn-extract btn-chip green" on:click={() => openExtractDialog(dip)}>
                    📥 Extract
                  </button>
                </td>
              </tr>
  ...
  <div class="modal-overlay" on:click={closeExtractDialog} role="dialog" aria-modal="true">
    <div class="modal-content modal" on:click|stopPropagation role="document">
      <div class="modal-header">
        <h3>📥 Extract Release</h3>
        <button class="modal-close" on:click={closeExtractDialog}>&times;</button>
      </div>
      <div class="modal-body">
        <p class="dip-info"><strong>{selectedReleaseForExtract?.title}</strong></p>
        <p class="info-text">Enter a password to encrypt the extracted Release package.</p>
        {#if extractError}
...
<style>
  ...
  .dip-title { font-weight: 600; color: var(--arc-ink); }
  .dip-description { font-size: 0.875rem; color: var(--arc-muted); }

  /* .badge base and the orange standard chip are global; these package status
  ...
  .dip-info { margin: 0 0 1rem 0; color: var(--arc-ink); }
