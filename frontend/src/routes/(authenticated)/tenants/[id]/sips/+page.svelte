...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    await Promise.all([loadTenant(), loadSips(), loadUsers()]);
  });
  ...
  async function handleExtract() {
    ...
    extractError = null;
    try {
      const response = await fetch(`${API_BASE}/api/archives/${selectedSipForExtract.id}/extract`, {
        method: 'POST',
...
<svelte:head>
  <title>{tenant ? `${tenant.displayName || tenant.name} - ` : ''}SIPs - Archiving System</title>
</svelte:head>
...
<div class="tenant-sips-page">
  {#if !hasAccess && !loading}
    <div class="access-denied">
      ...
      <h1>Access Denied</h1>
      <p>You don't have permission to view these SIPs.</p>
