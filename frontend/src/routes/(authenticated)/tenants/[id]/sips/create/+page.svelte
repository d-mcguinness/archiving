<script lang="ts">
  ...
  import { GET_ALL_USERS, CREATE_SIP, GET_ALL_SIPS_V2, GET_ARCHIVES_BY_TENANT, GET_TENANT, PREFILL_SIP_FIELDS } from '$lib/graphql/queries';
  ...
  onMount(async () => {
    ...
    // Guard: only ADMIN and TENANT (matching tenant) can create SIPs
    if (currentRole === 'ADMIN') {
      ...
      hasAccess = false;
      loading = false;
      toasts.error('You do not have permission to create SIPs');
      goto(`/tenants/${data.tenantId}/sips`);
      return;
    }
  ...
  async function onArchiveSelect() {
    ...
    sipTitle = selectedArchive.title ? `SIP - ${selectedArchive.title}` : '';
  ...
  function genId() {
    return `SIP-${selectedArchiveId || Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
  }
  ...
  function getStandardDefaults(standardKey: string, title: string, description: string): Record<string, string> {
    ...
    const map: Record<string, Record<string, string>> = {
      ...
      'OAIS': { packageID: id, title: title || 'New Submission Information Package', description: description || '', submissionDate: today, producer: userName, producerContact: user?.email || '', submissionAgreementRef: `SA-${id}`, packageType: 'SIP', contentInformationType: 'Digital', completeness: 'Complete', numberOfObjects: '1', totalSize: '0' },
  ...
  function applyAllDefaults() {
    ...
    if (!sipTitle) sipTitle = `${selectedStandard?.label || 'Archive'} SIP - ${new Date().toLocaleDateString()}`;
    if (!sipDescription) sipDescription = `Submission Information Package created using the ${selectedStandard?.label || 'selected'} standard.`;
  ...
  async function handleFilesProcessed(e: CustomEvent<{ files: File[], metadata: any }>) {
    ...
    try {
      ...
      for (const { name, value } of prefilled) {
        if (sipEntityDef?.fields?.some((f: any) => f.name === name)) {
          fieldValues[name] = value;
        }
      }
      fieldValues = fieldValues;
      ...
      if (files.length === 1 && metadata.filename) {
        ...
        if (!sipTitle || sipTitle.startsWith('SIP -')) sipTitle = `SIP - ${baseName}`;
      }
      ...
      const result = await client.mutate({
        ...
        variables: {
          input: {
            ...
            elementIdentifier, entityName: sipEntityDef.name, entityType: sipEntityDef.type,
...
<svelte:head><title>Create SIP - {tenant?.displayName || tenant?.name || 'Tenant'} - Arcana</title></svelte:head>

{#if !hasAccess && !loading}
  <div class="access-denied">
    <div class="access-denied-icon">🚫</div>
    <h1>Access Denied</h1>
    <p>You don't have permission to create SIPs.</p>
...
{:else}
  <div class="sip-container">
    <Breadcrumb
      ...
      items={[
        { label: 'SIPs', href: `/tenants/${data.tenantId}/sips` },
        { label: 'Create' }
      ]}
    />
    ...
    {:else}
      <form on:submit|preventDefault={handleSubmit}>
        ...
        {#if selectedArchiveId}
          ...
          {#if selectedStandard}
            <section class="form-section">
              ...
              <p class="section-hint">Drop files to auto-populate SIP metadata fields from file properties.</p>
          ...
          {:else if sipEntityDef}
            <section class="form-section">
              <h2 class="section-title">3. {selectedStandard?.sipLabel} Fields</h2>
              ...
              {#if sipEntityDef.note}
                <div class="entity-note">{sipEntityDef.note}</div>
              {/if}
        ...
        <div class="form-actions">
          ...
          <button type="submit" class="btn btn-primary" disabled={submitting || !selectedArchiveId || !sipEntityDef}>
