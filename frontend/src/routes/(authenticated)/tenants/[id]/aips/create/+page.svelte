<script lang="ts">
  ...
  import { GET_ALL_USERS, CREATE_AIP, GET_ALL_AIPS, GET_ARCHIVES_BY_TENANT, GET_TENANT, GET_SIPS_BY_TENANT_V2, PREFILL_SIP_FIELDS } from '$lib/graphql/queries';
  ...
  onMount(async () => {
    ...
    if (currentRole === 'ADMIN') {
      ...
      hasAccess = false;
      loading = false;
      toasts.error('You do not have permission to create AIPs');
      goto(`/tenants/${data.tenantId}/aips`);
      return;
    }
    ...
    try {
      const [tenantResult, archivesResult, usersResult, sipsResult] = await Promise.all([
        ...
        client.query({ query: GET_SIPS_BY_TENANT_V2, variables: { tenantId: data.tenantId }, fetchPolicy: 'network-only' })
      ]);
  ...
  async function onArchiveSelect() {
    ...
    aipTitle = selectedArchive.title ? `AIP - ${selectedArchive.title}` : '';
  ...
  function genId() {
    return `AIP-${selectedArchiveId || Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
  }
  ...
  async function handleFilesProcessed(e: CustomEvent<{ files: File[], metadata: any }>) {
    ...
    try {
      ...
      const result = await client.query({
        query: PREFILL_SIP_FIELDS,
        variables: { standard: selectedStandard.graphql, fileMetadata },
        fetchPolicy: 'network-only'
      });

      const prefilled = result?.data?.prefillSipFields || [];
      for (const { name, value } of prefilled) {
        if (aipEntityDef?.fields?.some((f: any) => f.name === name)) {
          fieldValues[name] = value;
        }
      }
      fieldValues = fieldValues;
      ...
      if (files.length === 1 && metadata.filename) {
        ...
        if (!aipTitle || aipTitle.startsWith('AIP -')) aipTitle = `AIP - ${baseName}`;
      }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const input: any = {
        ...
        elementIdentifier, entityName: aipEntityDef.name, entityType: aipEntityDef.type,
        elementTitle: aipTitle, elementDescription: aipDescription || null,
        createdBy: selectedUserId, fields
      };

      if (selectedSipId) {
        input.sourceSipId = selectedSipId;
      }
...
<svelte:head><title>Create AIP - {tenant?.displayName || tenant?.name || 'Tenant'} - Arcana</title></svelte:head>

{#if !hasAccess && !loading}
  <div class="access-denied">
    <div class="access-denied-icon">🚫</div>
    <h1>Access Denied</h1>
    <p>You don't have permission to create AIPs.</p>
...
{:else}
  <div class="aip-container">
    <Breadcrumb
      ...
      items={[
        { label: 'AIPs', href: `/tenants/${data.tenantId}/aips` },
        { label: 'Create' }
      ]}
    />
      ...
      <form on:submit|preventDefault={handleSubmit}>
        ...
        {#if selectedArchiveId}
          <section class="form-section">
            ...
            <div class="form-group">
              <label for="sourceSip">Source SIP (optional)</label>
              <select id="sourceSip" bind:value={selectedSipId}>
                <option value="">-- No source SIP --</option>
                {#each filteredSips as sip}
                  <option value={sip.id}>#{sip.id} - {sip.title} ({sip.status})</option>
                {/each}
              </select>
              <span class="field-hint">Optionally link this AIP to a source SIP.</span>
            </div>
          </section>
          ...
          {#if selectedStandard}
            <section class="form-section">
              ...
              <p class="section-hint">Drop files to auto-populate AIP metadata fields from file properties.</p>
          ...
          {:else if aipEntityDef}
            <section class="form-section">
              <h2 class="section-title">3. {selectedStandard?.aipLabel} Fields</h2>
              ...
              {#if aipEntityDef.children && aipEntityDef.children.length > 0}
                <div class="children-info">
                  ...
                  <p class="helper-text">Click to create AIP and add a child element:</p>
        ...
        <div class="form-actions">
          ...
          <button type="submit" class="btn btn-primary" disabled={submitting || !selectedArchiveId || !aipEntityDef}>
