<script lang="ts">
  ...
  import { GET_ALL_USERS, CREATE_DIP, GET_ALL_DIPS, GET_ARCHIVES_BY_TENANT, GET_TENANT, GET_AIPS_BY_TENANT, PREFILL_SIP_FIELDS } from '$lib/graphql/queries';
  ...
  onMount(async () => {
    ...
    if (currentRole === 'ADMIN') {
      ...
      hasAccess = false;
      loading = false;
      toasts.error('You do not have permission to create DIPs');
      goto(`/tenants/${data.tenantId}/dips`);
      return;
    }
    ...
    try {
      const [tenantResult, archivesResult, usersResult, aipsResult] = await Promise.all([
        ...
        client.query({ query: GET_AIPS_BY_TENANT, variables: { tenantId: data.tenantId }, fetchPolicy: 'network-only' })
      ]);
  ...
  async function onArchiveSelect() {
    if (!selectedArchiveId) {
      selectedArchive = null; selectedStandard = null; dipEntityDef = null; schemaData = null;
      fieldValues = {}; dipTitle = ''; dipDescription = ''; selectedUserId = ''; selectedAipId = '';
    ...
    dipTitle = selectedArchive.title ? `DIP - ${selectedArchive.title}` : '';
  ...
  async function loadSchema() {
    ...
    try {
      ...
      error = e instanceof Error ? e.message : 'Failed to load schema'; dipEntityDef = null;
    } finally { loadingSchema = false; }
  }

  function genId() { return `DIP-${selectedArchiveId || Date.now()}-${Math.random().toString(36).substring(2, 8)}`; }
  ...
  async function handleFilesProcessed(e: CustomEvent<{ files: File[], metadata: any }>) {
    ...
    try {
      ...
      for (const { name, value } of (result?.data?.prefillSipFields || [])) {
        if (dipEntityDef?.fields?.some((f: any) => f.name === name)) fieldValues[name] = value;
      }
      fieldValues = fieldValues;
      if (files.length === 1 && metadata.filename) {
        const baseName = metadata.filename.replace(/\.[^.]+$/, '');
        if (!dipTitle || dipTitle.startsWith('DIP -')) dipTitle = `DIP - ${baseName}`;
      }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await client.mutate({ mutation: CREATE_DIP, variables: { input }, refetchQueries: [{ query: GET_ALL_DIPS }], awaitRefetchQueries: true });
  ...
  function handleCancel() { goto(`/tenants/${data.tenantId}/dips`); }
</script>

<svelte:head><title>Create DIP - {tenant?.displayName || tenant?.name || 'Tenant'} - Arcana</title></svelte:head>

{#if !hasAccess && !loading}
  <div class="access-denied"><div class="access-denied-icon">🚫</div><h1>Access Denied</h1><p>You don't have permission to create DIPs.</p><p class="redirect-message">Redirecting...</p></div>
{:else}
  <div class="dip-container">
    <Breadcrumb context={{ tenantId: data.tenantId, tenantName: tenant?.displayName || tenant?.name }} items={[{ label: 'DIPs', href: `/tenants/${data.tenantId}/dips` }, { label: 'Create' }]} />

    <div class="dip-header"><div class="dip-header-top"><div><h1>Create Dissemination Information Package</h1>{#if tenant}<p class="subtitle">Tenant: {tenant.displayName || tenant.name}</p>{/if}</div>
    ...
    {:else}
      <form on:submit|preventDefault={handleSubmit}>
        ...
        {#if selectedArchiveId}
          ...
          {#if selectedStandard}
            <section class="form-section">
              ...
              <p class="section-hint">Drop files to auto-populate DIP metadata fields from file properties.</p>
          ...
          {:else if dipEntityDef}
            <section class="form-section">
              <h2 class="section-title">3. {selectedStandard?.dipLabel} Fields</h2>
        ...
        <div class="form-actions"><button type="button" class="btn btn-secondary" on:click={handleCancel}>Cancel</button><button type="submit" class="btn btn-primary" disabled={submitting || !selectedArchiveId || !dipEntityDef}>{submitting ? 'Creating DIP...' : 'Create DIP'}</button></div>
      </form>
    {/if}
  </div>
{/if}
