...
/**
 * Verifies the premium spend-cap check runs INSIDE createPreservation's transaction
 ...
 */
class PreservationServicePremiumCapTest {

    private static final Long TENANT = 100L;

    private final PreservationRepository repo = mock(PreservationRepository.class);
    private final UserApi userApi = mock(UserApi.class);
    private final PremiumOverageGuard guard = mock(PremiumOverageGuard.class);
    private final PreservationService service =
            new PreservationService(repo, userApi, mock(PreservationGeneratorFactory.class), guard);

    private CreatePreservationInput input(ArchiveStandard standard, boolean billable) {
        CreatePreservationInput in = new CreatePreservationInput();
        in.setUserId(2L);
    ...
    private void stubUserAndSave() {
        ...
        when(repo.save(any(Preservation.class))).thenAnswer(i -> i.getArgument(0));
    }
    ...
    void billablePremiumOverCapIsBlockedBeforeInsert() {
        ...
        assertThatThrownBy(() -> service.createPreservation(input(ArchiveStandard.NOARK5, true)))
    ...
    void billablePremiumWithinCapIsSavedAndRecorded() {
        ...
        assertThatCode(() -> service.createPreservation(input(ArchiveStandard.NOARK5, true)))
                .doesNotThrowAnyException();

        verify(guard).checkCanCreatePremiumPackage(TENANT);
        verify(repo).save(any(Preservation.class));
        // The generation is recorded to the append-only ledger after the save.
        verify(guard).recordPremiumPackageGenerated(TENANT, "NOARK5", PremiumPackageType.PRESERVATION);
    }
    ...
    void nonPremiumStandardSkipsTheGuard() {
        ...
        service.createPreservation(input(ArchiveStandard.OAIS, true));
    ...
    void operatorCreatedPremiumSkipsTheGuard() {
        ...
        service.createPreservation(input(ArchiveStandard.NOARK5, false)); // billable=false (ADMIN)
    ...
    void nullTenantIdIsRejected_neverBilledToTheOwner() {
        ...
        CreatePreservationInput in = input(ArchiveStandard.NOARK5, true);
        in.setTenantId(null); // no resolved billing tenant — must fail, not fall back to ownerId

        assertThatThrownBy(() -> service.createPreservation(in))
